...
export default {
    ...
    methods:{
        ...
        upload(){
            let _self = this
            Bus.$emit('upload_material_house',[_self.resourceType,"learn"])
        },
        edit(){
            let _self = this
            if(this.local_rows != ""){
                Bus.$emit('Open_material_house_edit', [_self.local_rows, _self.resourceType,"learn"])
            }else{
        ...
        getData(){
            this.local_rows = []
            let _self = this
            let url = `api/system/resource/learn/source/list`
            let config = {
                params:{
                    page:_self.page,
                    pageSize: 10,
