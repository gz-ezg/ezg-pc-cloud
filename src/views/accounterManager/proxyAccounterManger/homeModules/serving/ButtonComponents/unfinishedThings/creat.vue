...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            editable(content,id){
                let _self = this
                _self.$bus.$emit("AMEND_UNDO_PHRASE_DATA",content,id)
            },
            selectArr(id){
            ...
            add_schtask(){
                this.$bus.emit("ADD_UNDO_SCHTASK")
            },
            delete_phrase_list(id){
                ...
                function success(res){
                    _self.phraseLoading = false
                    _self.$bus.emit("UPDATE_UNDO_PHRASE_LIST", true)
                }
                this.$Get(url, config, success)
            },
        },
        created() {
            this.$bus.off("OPEN_ADD_UNFINISHED_THINGS",true)
            this.$bus.on("OPEN_ADD_UNFINISHED_THINGS",(e)=>{
            ...
            this.$bus.off("UPDATE_UNDO_PHRASE_LIST",true)
            this.$bus.on("UPDATE_UNDO_PHRASE_LIST",(e)=>{
                this.get_phrase_list()
            })
        }
    }
