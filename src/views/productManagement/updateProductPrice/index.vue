    ...
    <Card style="min-width:1400px">
      ...
      <div v-for="(item, index) in queryProperty" :key="index" style="margin-top:10px">
        <Row style="display:flex;justify-content:flex-start">
          <!-- <Col span="4"> -->
          <div style="padding-right:20px;padding-left:10px;width:120px;display:inline-block;text-align:right;line-height:32px;">
            {{ item.name }} :
          </div>
          <!-- </Col> -->
          <!-- <Col span="20"> -->
          <RadioGroup v-model="typeArray[index]" type="button">
            <Radio @click="handleClick" v-for="(item2, index2) in item.children" :label="item2.pvId" :key="index2">{{
              item2.propertyValue
            }}</Radio>
          </RadioGroup>
          <!-- </Col> -->
      ...
      <Row style="margin-top:10px;margin-left:10px" v-if="showUpdatePrice">
        <div style="display:flex;justify-content:flex-start;align-items:center">
          <span>基础价格：</span><Input style="width:150px;margin-right:40px;" v-model="dbaseprice" /> <span>系统价格：</span
          ><Input style="width:150px;margin-right:40px;" v-model="doaprice" />
          <span>状态：</span>
      ...
      <Row :gutter="20" style="margin-top:10px">
        ...
        <Col span="24" v-else>
          <tree-table selection :data="data" :columns="columns" border @get-all-selection="get_all_selection" />
        </Col>
      </Row>
    </Card>
  </div>
  ...
  methods: {
    ...
    get_product_attr(e) {
      ...
      function success(res) {
        _self.queryProperty = res.data.data.map(v => {
          v.children.unshift({ ppvId: 0, propertyValue: '无', pvId: 0, selected: 0 });
          return v;
        });
