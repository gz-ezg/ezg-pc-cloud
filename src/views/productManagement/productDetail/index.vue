            ...
            <Row :gutter="20">
                ...
                <Col span="12">
                    ...
                    <Row v-for="(item, index) in propertys" :key="index" style="margin-top:10px">
                        <Row><h3 style="display:inline-block;float:left">属性值 - {{index+1}}：{{item.name}}</h3><Button style="display:inline-block;float:right" type="warning" size="small" @click="delete_product_type(index)">删除</Button></Row>
    ...
    data() {
        return {
            ...
            ruleValidate: {
                ...
                departalias: [
                    { required: true, message: '服务部门不能为空！', trigger: 'change' , type: 'array', min: 1 }
                ],
                productCode: [
