<template>
  <div class='autoTbale'>
        <table class="table table-bordered" id='hl-tree-table' :style="'min-width:'+this.tdsWidth +'+px'">
            <thead>
                <tr>
                    <th v-for="(column,index) in cloneColumns" :key="index" :style="'width:'+ column._width +'px;'">
            ...
            <tbody>
                <tr v-for="(item,index) in initItems" :key="item.id" v-show="show(item)" :class="{'child-tr':item.parent}" @click="trclick(item)">
                    <td v-for="(column,snum) in columns" :key="column.key" :style=tdStyle(column) >
                        ...
                        <div v-if="column.type === 'action'">
                          <i-button :type="action.type" style="margin:2px 10px" size="small" @click="RowClick(item,$event,index,action.text)" v-for='action in (column.actions)' :key="action.text">{{action.text}}</i-button>
                        </div>
                        <label @click="toggle(index,item)" v-if="!column.type">
                            <span v-if='snum==iconRow()'>
                              <i v-html='item.spaceHtml'></i>
                              <i v-if="item.children&&item.children.length>0" class="ivu-icon" :class="{'ivu-icon-chevron-down':!item.expanded,'ivu-icon-chevron-up':item.expanded }"></i>
  ...
  export default {
    ...
    methods: {
      ...
      RowClick (data, event, index, text) {
        //  data 这一行的数据，event鼠标事件，index序号，text按钮标题
        console.log(data)
        let result = this.makeData(data)
        this.$emit('on-row-click', result, event, index, text)
      },
      trclick(data){
        this.$emit('tr-click',data)
      },
        // 点击事件 返回数据处理
      makeData (data) {
        const t = typeof data
      ...
      makeColumns () {
        ...
        columns.forEach((column, index) => {
          ...
          this.tdsWidth += column.width ? parseFloat(column.width) : '1400px'
        })
        return columns
      },
      // 数据处理 增加自定义属性监听
...
#hl-tree-table>tbody>.child-tr>td:nth-child(2) {
  text-indent: 2em
}
label {
    margin: 0 8px;
