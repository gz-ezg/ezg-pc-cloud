<template>
  <div>
    <Modal v-model="showModal" title="编辑" width="1000" @on-cancel="close">
    ...
    <Card>
      ...
      <Table :loading="userTableLoading" highlight-row :columns="header" :data="data" @on-row-click="selectRow"></Table>
      <Page
        size="small"
        :total="total"
        show-total
...
export default {
  ...
  data() {
    return {
      filtraConfig: [
        { type: 'input', key: 'realname', label: '用户名称：' },
        { type: 'input', key: 'departname', label: '部门名称：' },
        { type: 'select', key: 'post', label: '岗位' },
        { type: 'input', key: 'order_receiving', label: '是否接单：' }
      ],
      search_model: '',
      page: 1,
      pageSize: 10,
