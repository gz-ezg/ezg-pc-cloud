...
<template>
  <div style="min-width:800px">
      ...
      <TabPane label="未领取" name="name1">
        <Card>
                  ...
                  <Form ref="SearchValidate" :model="SearchValidate" :label-width="80">
                    ...
                    <center>
                      <FormItem style="margin-top:10px">
                        ...
                        <Button
                          type="ghost"
                          @click="handleReset('SearchValidate')"
          ...
          <Row>
            <ButtonGroup>
              <Button
                v-permission="['cluesLibraryM.n.add']"
                type="primary"
              ...
              <Button
                v-permission="['cluesLibraryM.n.delete']"
                type="primary"
                icon="ios-color-wand-outline"
                @click="DeleteClues"
              >删除</Button>
      ...
      <TabPane label="已领取" name="name2">
        <Card>
                  ...
                  <Form ref="SearchValidate1" :model="SearchValidate1" :label-width="80">
                    ...
                    <center>
                      <FormItem style="margin-top:10px">
                        ...
                        <Button
                          type="ghost"
                          @click="handleReset1('SearchValidate1')"
                          style="margin-left: 8px"
                        >重置</Button>
                      </FormItem>
                    </center>
          ...
          <Row>
            <ButtonGroup>
              <Button
                v-permission="['cluesLibraryM.y.delete']"
                type="primary"
                icon="ios-color-wand-outline"
                @click="DeleteClues"
              >删除</Button>
      ...
      <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="90">
        ...
        <FormItem label="公司地区" prop="companyname">
          <Select v-model="formValidate.affiliationArea" size="small">
            <Option
              v-for="(item, index) in affiliation_area"
              :value="item.typecode"
              :key="index"
            >{{item.typename}}</Option>
          </Select>
        ...
        <FormItem label="客户电话" prop="customerTel">
          <Input
            v-model="formValidate.customerTel"
            size="small"
            @on-focus="get_customer"
            :readonly="isOldCustomer"
          ></Input>
        ...
        <FormItem label="标签" prop="labels">
          <Tag
            v-for="item in customerlabelGroup"
            :key="item"
            :name="item"
            :id="item.id"
            closable
          ...
          <Button
            ...
            type="dashed"
            size="small"
            @click="getLabelData();addTag = true"
          >添加</Button>
        </FormItem>
        <FormItem label="线索说明" prop="clueMemo">
          <Input
            v-model="formValidate.clueMemo"
            type="textarea"
    ...
    >
      <Form ref="formValidate22" :model="formValidate22" :rules="ruleValidate22" :label-width="90">
        ...
        <FormItem label="运营来源" prop="labels">
          <Tag
            v-for="item in customerlabelGroup22"
            :key="item"
            :name="item"
            :id="item.id"
            closable
        ...
        <FormItem label="线索说明" prop="clueMemo">
          <Input
            v-model="formValidate22.clueMemo"
            type="textarea"
            :autosize="{minRows: 2,maxRows: 5}"
            size="small"
          >></Input>
    ...
    >
      <Form ref="formValidate2" :model="formValidate2" :label-width="90">
        ...
        <FormItem label="线索说明" prop="cluescontent">
          <Input
            v-model="formValidate2.cluescontent"
            type="textarea"
            :autosize="{minRows: 2,maxRows: 5}"
          ></Input>
        </FormItem>
    ...
    <Modal v-model="modal4" title="查看线索" class-name="vertical-center-modal" :styles="{top: '10%'}">
      <Form ref="formValidate2" :model="formValidate2" :label-width="90">
        <FormItem label="客户名称" prop="customerName">
          <Input v-model="formValidate2.customerName" disabled size="small">></Input>
        </FormItem>
    ...
    <Modal v-model="addTag" title="添加标签" @on-ok="ok2" @on-cancel="cancel2">
      <Table
        border
        ref="selection"
        :columns="columns4"
    ...
    <Modal v-model="selectCompany" title="选择公司" width="80%">
      <Row :gutter="16">
        <Col span="8">
    ...
    <public-customer
      v-if="openPublicCustomer"
      :customer="currnetRow"
      @close="openPublicCustomer = false"
...
export default {
  ...
  methods: {
    ...
    handleReset() {
      ...
      (_self.SearchValidate.date = ""), (_self.SearchValidate.clueType = "");
      ...
      function doSuccess(response) {
        ...
        for (let i = 0; i < response.data.data.rows.length; i++) {
          ...
          _self.tableData.push({
            ...
            cluestype: _self.findCluetypeText(
              response.data.data.rows[i].clue_type
            ),
            ...
            clue_level: _self.clue_level_map.get(
              response.data.data.rows[i].clue_level
            ),
            ...
            settlement_status: simpleCodeToText(
              response.data.data.rows[i].settlement_status,
              _self.settlement_status
            )
          });
        }
    ...
    detail(e) {
      let _self = this;
      let url =
        "/order/queryListByCustomer?customerId=" +
        e.row.oacustomerid +
    ...
    pageChange2(a) {
      var _self = this;
      this.$http
        .get("/api/system/label/list?page=" + a + "&pageSize=10")
        .then(function(data) {
          _self.data = [];
          var response = data.data.data;
          _self.pageTotal2 = response.total;
    ...
    pageSizeChange2(a) {
      var _self = this;
      this.$http
        .get("/api/system/label/list?page=1&pageSize=" + a)
        .then(function(data) {
          _self.data = [];
          var response = data.data.data;
          _self.pageTotal2 = response.total;
