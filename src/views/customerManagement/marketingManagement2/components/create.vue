    ...
    >
      <Form ref="formValidate" :rules="ruleValidate" :model="formValidate" :label-width="80">
        ...
        <Row :gutter="16" v-if="customerShow">
          <Col span="12">
            <FormItem label="关联客户" prop="recCustomer">
              <!-- <Input size="small" v-model="formValidate.recCustomer" @on-focus="getCustomer" readonly />               -->
              <Row>
                <Input size="small" v-model="formValidate.recCustomer" @on-focus="showCustomerPopus" />
                <Button size="small" @click="clearResCustomer">清空</Button>
              </Row>
            </FormItem>
          </Col>
        </Row>
        ...
        <Row :gutter="16">
          <Col span="24">
            <FormItem label="标签" prop="customerTags" style="margin-bottom:10px">
              ...
              <Button
                name="marketingManagement_index_entry_add"
                icon="ios-plus-empty"
                type="dashed"
                size="small"
                @click="open_tag"
  ...
  data() {
    ...
    return {
      ...
      ruleValidate: {
        ...
        channelTypeId: [{ validator: validateChannel, trigger: 'change', message: '请选择渠道来源' }],
  ...
  methods: {
    ...
    create_customer() {
      ...
      let config = {
        ...
        recCustomer: _self.formValidate.recCustomer,
        recommendCustomerId: _self.formValidate.recCustomerId
      };
    ...
    clearResCustomer() {
      ...
      this.formValidate.recCustomerId = '';
    },
    showCustomerPopus() {
      this.$bus.emit('OPEN_CUSTOMER', this.formValidate);
    },
    //  检查客户来源类型
    check_channel(e) {
      //  渠道推荐
