    ...
    <Card>
              ...
              >
                ...
                <Row :gutter="6" class="row">
                  ...
                  <Col span="6">
                    <FormItem label="客户来源：" prop="customersource" style="margin-left: 3px">
                      <Select
                        transfer
                        v-model="formValidate.customersource"
                        size="small"
                        @on-change="search"
                        style="width: 160px"
                      >
                  ...
                  <Col span="6">
                    <FormItem label="渠道名称：" prop="channelTypeName" style="margin-left: 5px">
                      <Input
                        v-model="formValidate.channelTypeName"
                        size="small"
                        style="width: 158px;"
                      ></Input>
                    </FormItem>
               ...
               <Col span="6">
                    <FormItem label="客户重要性：" prop="importance">
                      <Select
                        transfer
                        v-model="formValidate.importance"
                        size="small"
                        @on-change="search"
                        style="width: 158px"
                      >
                ...
                <Row :gutter="12" class="row">
                  ...
                  <Col span="6">
                    <FormItem label="剩余跟进时间：" prop="begin_residue_time">
                      <Input
                        v-model="formValidate.begin_residue_time"
                        size="small"
                        style="width:30%"
                        placeholder="起始"
                      ></Input>-
                      <Input
                        v-model="formValidate.end_residue_time"
                        size="small"
                        style="width:30%"
                        placeholder="结束"
                      ></Input>
                    ...
                    <FormItem label="标签：" prop="labelName">
                      <Select
                        size="small"
                        v-model="formValidate.labelName"
                        multiple
                        style="width:160px"
                      >
                        <Option
      ...
      <Row>
        <ButtonGroup>
          ...
          <Button
            type="primary"
            name="marketingManagement_index_edit_qrcode"
            icon="grid"
            @click="check_select('OPEN_CODE')"
          >查看二维码</Button>
          <Button
            type="primary"
            name="marketingManagement_index_edit_excel"
            icon="ios-download"
            @click="download_excel"
          >导出Excel</Button>
          <Button
            type="primary"
            name="marketingManagement_index_info_log"
            icon="drag"
            @click="check_select('OPEN_CHANGE_LOG')"
          >销售变更日志</Button>
          <Button
            type="primary"
...
export default {
  ...
  methods: {
    change_format() {
      let a = FULLDateFormat(this.formValidate.credate[0])
        .split(" ")[1]
        .split(":")
        .join("");
      let b = FULLDateFormat(this.formValidate.credate[1])
        .split(" ")[1]
        .split(":")
        .join("");
      let c = a + b;
    ...
    change_format1() {
      let a = FULLDateFormat(this.formValidate.upddate[0])
        .split(" ")[1]
        .split(":")
        .join("");
      let b = FULLDateFormat(this.formValidate.upddate[1])
        .split(" ")[1]
        .split(":")
        .join("");
      let c = a + b;
    ...
    download_excel() {
      ...
      if (_self.formValidate.upddate[0] && !_self.formValidate.credate[0]) {
        let a = FULLDateFormat(_self.formValidate.upddate[0])
          .split(" ")[1]
          .split(":")
          .join("");
        let b = FULLDateFormat(_self.formValidate.upddate[1])
          .split(" ")[1]
          .split(":")
          .join("");
        let c = a + b;
        ...
        if (c == "000000000000") {
          ...
          let config = {
            ...
            labelName: _self.formValidate.labelName.join(','),
        ...
        _self.formValidate.credate[0] &&
        !_self.formValidate.upddate[0]
      ) {
        let a = FULLDateFormat(_self.formValidate.credate[0])
          .split(" ")[1]
          .split(":")
          .join("");
        let b = FULLDateFormat(_self.formValidate.credate[1])
          .split(" ")[1]
          .split(":")
          .join("");
        let c = a + b;
        ...
        let b = FULLDateFormat(_self.formValidate.upddate[1])
          ...
          .split(":")
          .join("");
        let c = FULLDateFormat(_self.formValidate.credate[0])
          .split(" ")[1]
          .split(":")
          .join("");
        let d = FULLDateFormat(_self.formValidate.credate[1])
          .split(" ")[1]
          .split(":")
          .join("");
        let e = a + b;
          ...
          function success(res) {
            ...
            for (let i = 0; i < _self.data.length; i++) {
              ...
              if (_self.data[i].importlevel) {
                _self.data[i].importlevel = _self.customerrating_map.get(
                  _self.data[i].importlevel.toString()
                );
              }
              ...
              _self.data[i].importance = _self.importance_map.get(
                _self.data[i].importance
              );
              _self.data[i].isbound = _self.sf_yn_map.get(
                _self.data[i].isbound
              );
              if (_self.data[i].CREATEDATE) {
                _self.data[i].CREATEDATE = _self.data[i].CREATEDATE.slice(
                  0,
                  10
                );
              }
              if (_self.data[i].updatedate) {
                _self.data[i].updatedate = _self.data[i].updatedate.slice(
                  0,
                  10
                );
              }
    ...
    row_class_name(row, index) {
      if (
        (row.residue_time <= 7 || typeof row.residue_time == "string") &&
        row.residue_time != null
      ) {
