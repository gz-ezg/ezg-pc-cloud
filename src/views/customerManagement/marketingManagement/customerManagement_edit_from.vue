    ...
    <div>
        <Card>
            <Form ref="formValidate" :rules="ruleValidate" :model="formValidate" :label-width="80">
                ...
                <FormItem label="标签" prop="customerlabelTags" style="margin-bottom:0px">
                    ...
                    <Button name="marketingManagement_index_edit_addtag" icon="ios-plus-empty" type="dashed" size="small" @click="addTag">添加</Button>
                ...
                <FormItem style="margin-bottom:0px">
                    ...
                    <Button type="primary" name="marketingManagement_index_edit_save" @click="handleSubmit('formValidate')" style="margin-left: 20px">保存</Button>                  
                </FormItem>
            </Form>
        </Card>
        <Modal
        ...
        >
            <Row :gutter="16">
                ...
                <Col span="4">
                    <Button name="marketingManagement_index_edit_recommend_client" icon="ios-search" @click="getCustomer">搜索</Button>
                </Col>
            </Row>
            <Table
...
export default {
  ...
  data() {
    ...
    return {
      ...
      ruleValidate: {
        ...
        customersource: [{ required: true, trigger: "change" }],
        importlevel: [{ required: true, trigger: "change" }],
  ...
  methods: {
    ...
    getAllLabel() {
      ...
      this.$http
        .get("/api/system/label/list?page=1&pageSize=1000")
        .then(function(LabelsAllRes) {
          for (let i = 0; i < LabelsAllRes.data.data.rows.length; i++) {
            ...
            areaObj.channelflag =
              LabelsAllRes.data.data.rows[i].channelFlag == "0" ? "否" : "是";
            areaObj._checked = "";
    ...
    getData() {
      ...
      this.$http
        .all([
          ...
          this.$http.get(
            "/api/customer/findAllCustomerLabelsByCustomerId/" +
              _self.customerid
          )
        ])
        .then(
          this.$http.spread(function(customerdetailResp, LabelsRes) {
                ...
                if (temp[1] == "" || temp[1] == null) {
                  if (temp[0] == _self.customerTypeArr[i].id) {
                    if (_self.customerTypeArr[i].pid == 0) {
                      _self.formValidate.customertype.push(
                        _self.customerTypeArr[i].id
                      );
                    } else {
                      _self.formValidate.customertype.push(
                        _self.customerTypeArr[i].pid,
                        _self.customerTypeArr[i].id
                      );
                    }
                  }
    ...
    handleSubmit(name) {
      ...
      var _self = this;
      _self.$ButtonCollect("marketingManagement_index_edit_save");
      this.$refs[name].validate(valid => {
        ...
        if (valid) {
          if (
            (_self.formValidate.tel == "" || _self.formValidate.tel == null) &&
            (_self.formValidate.fixedphone == "" ||
              _self.formValidate.fixedphone == null) &&
            (_self.formValidate.qq == "" || _self.formValidate.qq == null) &&
            (_self.formValidate.weixin == "" ||
              _self.formValidate.weixin == null)
          ) {
            this.$nextTick(() => {
              this.loading = true;
            });
    ...
    cancel() {
      this.$ButtonCollect("marketingManagement_index_edit_addtag_cancel");
    },
    ok() {
      this.$ButtonCollect("marketingManagement_index_edit_addtag_save");
    },
    addTag() {
      let _self = this;
      _self.$ButtonCollect("marketingManagement_index_edit_addtag");
      _self.modal1 = true;
      // //console.log(this.data1)
      // //console.log(this.customerlabel)
      for (let i = 0; i < _self.data1.length; i++) {
        for (let j = 0; j < _self.customerlabel.length; j++) {
          if (_self.data1[i].id == _self.customerlabel[j].id) {
            _self.data1[i]._checked = true;
          }
        }
      }
    },
    ...
    getCustomer() {
      let _self = this;
      _self.$ButtonCollect("marketingManagement_index_edit_recommend_client");
