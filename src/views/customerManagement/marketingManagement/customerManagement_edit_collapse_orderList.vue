...
<template>
    <div>
        ...
        >
            ...
            <div slot="footer">
                <Button name="" type="primary" @click="ok4('formValidate')" :loading="add_order_button_loading">提交订单</Button>
            </div>
        </Modal>
        <Modal
                v-model="deleteOrder"
        ...
        <Modal
            ...
            <Form ref="formValidateDetail" :model="formValidateDetail" :label-width="100">
                ...
                <Row :gutter="16">
                    <FormItem>
                        <Button name="marketingManagement_index_orderlist_product_check" type="primary" icon="plus" @click="kuaiji()" v-show="kjdj">查看会计到家服务项</Button>
                    </FormItem>
                </Row>
        ...
        <Modal
            ...
            <Form ref="formValidateEadit" :rules="ruleValidateEadit" :model="formValidateEadit" :label-width="100">
                ...
                <Row :gutter="16">
                    <FormItem label="选择产品">
                        <Button name="marketingManagement_index_orderlist_product_add" type="primary" icon="plus" @click="orderAddProducts()">新增</Button>
                        <Button name="marketingManagement_index_orderlist_product_delect" type="primary" icon="plus" @click="removeRows()">删除</Button>
                        <Button name="marketingManagement_index_orderlist_product_check" type="primary" icon="plus" @click="kuaiji()" v-show="kjdj">查看会计到家服务项</Button>
                    </FormItem>
                </Row>
        ...
        <Modal
            ...
            <Form ref="formValidatexiugai" :rules="ruleValidatexg" :model="formValidatexiugai" :label-width="100">
                ...
                <Row :gutter="16">
                    <FormItem>
                        <Button name="marketingManagement_index_orderlist_invalid" type="primary" icon="plus" @click="zuofei()">作废订单项</Button>
                        <Button name="marketingManagement_index_orderlist_product_check" type="primary" icon="plus" @click="kuaiji()" v-show="kjdj">查看会计到家服务项</Button>
                    </FormItem>
                </Row>
        ...
        <Modal
            ...
            <Row :gutter="16">
                <Col span="8">
                    ...
                    <Button name="marketingManagement_index_orderlist_selectcompany" slot="append" icon="ios-search" @click="searchCompanyData"></Button>
                    </Input>
                </Col>
            </Row>
            <Table
            ...
            <Row style="margin-bottom:10px">
                <Col span="4">
                    <Input v-model="searchProduct" placeholder="输入产品搜索" @on-keydown.enter="searchProductData">
                        <Button name="marketingManagement_index_orderlist_product_search" slot="append" icon="ios-search" @click="searchProductData"></Button>
                    </Input>
                </Col>
            </Row>
            <Row>
                ...
                <Col span="11" offset="1">
                    <Card dis-hover v-if="isProductDetail">
                        ...
                        <div class="productDetail-option" id="productDetail-option-id" @click="areaChange()">
                            ...
                            <Button name="marketingManagement_index_orderlist_product_buy" type="error" icon="bag" size="large" style="margin-top: 20px" @click="addProduct">
                                立即购买
                            </Button>
                        </div>
                    </Card>
	...
	<Modal
            ...
            <div slot="footer"><Button name="marketingManagement_index_orderlist_taxperiod" type="primary" @click="Dateok">确定</Button></div>
        </Modal>
    </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      ruleValidate: {
        ...
        payDir: [{ required: true, message: "    ", trigger: "change" }],
        orderPayNumber: [{ required: true, message: "   " }],
                ...
                [
                  ...
                  h(
                    ...
                    [
                      h(
                        "ul",
                        this.data6[params.index].CompanyName.map(item => [
                          h(
                            "li",
                            {
                              style: {
  ...
  methods: {
      ...
      function doSuccess(response) {
        ...
        for (let i = 0; i < _rowsData.length; i++) {
          ...
          _self.data6.push({
            ...
            customersource: _self.cluesources_map.get(
              _rowsData[i].customersource
            )
          });
        }
      }
    ...
    detailCustomers(a) {
      let _self = this;
      _self.$ButtonCollect("marketingManagement_index_orderlist_check");
      if (_self.customerId == "") {
        ...
        function doSuccess(response) {
          ...
          (_self.formValidateDetail.GDSreport = _data.gdsreport),
            (_self.formValidateDetail.performanceMoney =
              _data.performanceMoney),
    ...
    searchProductData() {
      this.$ButtonCollect("marketingManagement_index_orderlist_product_search");
    ...
    orderAddProducts() {
      let _self = this;
      _self.$ButtonCollect("marketingManagement_index_orderlist_product_add");
      if (_self.searchProduct == "") {
        var url = "/product/list?page=1&pageSize=10";
      } else {
        var url =
          "/product/list?page=1&pageSize=10&product=" + _self.searchProduct;
      }
    ...
    pageSizeChange2(a) {
      ...
      _self.pageSize = a;
      this.$http
        .get("/api/product/list?page=1&pageSize=" + a)
    ...
    doProductSKU() {
      ...
      function doSuccess(response) {
        ...
        _self.resetProperty(
          _self.productSKU[0].linkPropertys,
          _self.productSKU[0].linkPropertyIds
        );
    ...
    setPrice(skuId, areaId) {
      ...
      if (!skuId) {
        ...
        let url =
          "/product/getPriceByAreaAndSKU?skuId=" +
          skuId +
          "&areaId=" +
          _self.res1[2];
    ...
    areaChange() {
      ...
      if (obj.areaId != _self.lastObj.areaId) {
        ...
        obj.linkPropertyValueIds.join(",") !=
        _self.lastObj.linkPropertyValueIds.join(",")
      ) {
        _self.lastObj = obj;
        let button = $("div[name='firstProperty']").find(
          "button[class='active']"
        );
      ...
      if (_self.isEdit != true) {
        ...
        for (let i = 0; i < _self.orderItemList.length; i++) {
          if (_self.orderItemList[i].plusdeduct == "N") {
            _self.formValidate.ticheng += Number(
              _self.orderItemList[i].paynumber
            );
          }
          _self.formValidate.zongjia += Number(
            _self.orderItemList[i].paynumber
          );
          _self.formValidate.orderPayNumber += Number(
            _self.orderItemList[i].paynumber
          );
          _self.formValidate.performanceMoney += Number(
            _self.orderItemList[i].paynumber
          );
        }
        ...
        for (let i = 0; i < _self.orderItemList2.length; i++) {
          if (_self.orderItemList2[i].plusdeduct == "N") {
            _self.formValidateEadit.ticheng += Number(
              _self.orderItemList2[i].paynumber
            );
          }
          _self.formValidateEadit.zongjia += Number(
            _self.orderItemList2[i].paynumber
          );
          _self.formValidateEadit.orderPayNumber += Number(
            _self.orderItemList2[i].paynumber
          );
          _self.formValidateEadit.performanceMoney += Number(
            _self.orderItemList2[i].paynumber
          );
        }
    ...
    addProduct() {
      let _self = this;
      _self.$ButtonCollect("marketingManagement_index_orderlist_product_buy");
      let url =
        "/order/queryItemDetail?productSkuIds=" +
        _self.SKU +
        "&areaId=" +
        _self.res1[2];
      ...
      function doSuccess(response) {
        ...
        if (
          response.data.data[0].iscycle != "N" &&
          response.data.data[0].product != "会计到家"
        ) {
          let _error = "";
          let url2 = "";
          if (_self.isEdit != true) {
            ...
            url2 =
              "/order/cycle/service/record/budget/period?productId=" +
              _self.productId +
              "&companyId=" +
              _self.formValidate.cpid;
          } else {
            ...
            url2 =
              ...
              _self.productId +
              "&companyId=" +
              _self.formValidateEadit.companyid;
          }
          if (_error) {
    ...
    ok4(name) {
      let _self = this;
      _self.$ButtonCollect("marketingManagement_index_orderlist_submit");
      setTimeout(() => {
        this.loading = false;
        this.$refs[name].validate(valid => {
          if (valid) {
            if (_self.iscycle == false || _self.isfuwu == true) {
              for (let i = 0; i < _self.orderItemList.length; i++) {
                if (
                  (_self.orderItemList[i].product.indexOf("代理记账") != -1 ||
                    _self.orderItemList[i].product.indexOf("会计到家") != -1) &&
                  _self.orderItemList[i].servicestartdate == ""
                ) {
                  _self.$nextTick(() => {
                    _self.loading = true;
                  });
            ...
            if (_self.iscycle == false && _self.formValidate.GDSreport == "") {
              ...
              if (_self.isfuwu == true) {
                let url =
                  "/system/serviceContract/template/queryTreegridByCode?templateCode=kjdafwxy&objectId=" +
                  _self.formValidate.cpid;
                function doSuccess(response) {
                  if (
                    response.data.data.selectItems == null ||
                    response.data.data.selectItems == ""
                  ) {
                    _self.$nextTick(() => {
                      _self.loading = true;
                    });
    ...
    editPostFrom(name) {
      let _self = this;
      _self.$ButtonCollect("marketingManagement_index_orderlist_edit_save");
      setTimeout(() => {
        this.loading2 = false;
        this.$refs[name].validate(valid => {
          if (valid) {
            ...
            for (let i = 0; i < _self.orderItemList2.length; i++) {
              if (
                _self.orderItemList2[i].product == "小规模代理记账" ||
                _self.orderItemList2[i].product == "0申报代理记账" ||
                _self.orderItemList2[i].product == "一般纳税人代理记账"
              ) {
                _jizhang = true;
                break;
              }
            ...
            if (_jizhang == true || _self._iscycle == true) {
              for (let i = 0; i < _self.orderItemList2.length; i++) {
                if (
                  (_self.orderItemList2[i].product.indexOf("代理记账") != -1 ||
                    _self.orderItemList2[i].product.indexOf("会计到家") !=
                      -1) &&
                  (_self.orderItemList2[i].servicestartdate == "" ||
                    _self.orderItemList2[i].servicestartdate == undefined)
                ) {
                  _self.$nextTick(() => {
                    _self.loading = true;
                  });
                ...
                function doSuccess(response) {
                  if (
                    ...
                    (_data.companyId = _self.formValidateEadit.companyid),
                      ...
                      (_data.orderPayNumber =
                        _self.formValidateEadit.orderPayNumber),
      ...
      setTimeout(() => {
        ...
        this.$refs[name].validate(valid => {
          if (valid) {
            ...
            for (let i = 0; i < _self.orderItemList4.length; i++) {
              if (
                _self.orderItemList4[i].product == "小规模代理记账" ||
                _self.orderItemList4[i].product == "0申报代理记账" ||
                _self.orderItemList4[i].product == "一般纳税人代理记账"
              ) {
                _jizhang = true;
                break;
              }
            ...
            if (
              _jizhang == true &&
              (_self.formValidatexiugai.serviceBeginDate == "" ||
                _self.formValidatexiugai.serviceBeginDate == null)
            ) {
              _self.$nextTick(() => {
                _self.loading3 = true;
              });
              ...
              if (_iscycle == true) {
                let url =
                  "/system/serviceContract/template/queryTreegridByCode?templateCode=kjdafwxy&objectId=" +
                  _self.formValidatexiugai.companyid;
    ...
    flowChart(a) {
      let _self = this;
      _self.$ButtonCollect("marketingManagement_index_orderlist_flowChart");
    ...
    getCompany() {
      ...
      if (_self.isSearch == false) {
        url =
          "/customer/company/list?page=1&pageSize=10&customerId=" +
          _self.customerid;
      } else {
        url =
          "/customer/company/list?page=1&pageSize=10&companyname=" +
          _self.searchCompany +
          "&customerId=" +
          _self.customerid;
      }
    ...
    pageSizeChange3(a) {
      ...
      if (_self.isSearch == false) {
        url =
          "/customer/company/list?page=1&pageSize=" +
          _self.pageSize +
          "&customerId=" +
          _self.customerid;
      } else {
    ...
    searchCompanyData() {
      let _self = this;
      _self.$ButtonCollect("marketingManagement_index_orderlist_selectcompany");
    ...
    reApplyProcess() {
      let _self = this;
      _self.$ButtonCollect("marketingManagement_index_orderlist_resubmit");
      if (_self.currentProcess == "Returned") {
        _self.$Modal.confirm({
          ...
          onOk: () => {
            let url =
              "/activiti/reApplyProcessByOrderId?orderId=" +
              _self.customerId +
              "&auditFlag=1";
    ...
    removeRows() {
      ...
      _self.$ButtonCollect("marketingManagement_index_orderlist_product_delect");
      let _form;
      if (_self.isEdit == true) {
        ...
        for (let i = 0; i < _self.orderItemList.length; i++) {
          if (
            _self.orderItemList[i].product != "小规模代理记账" &&
            _self.orderItemList[i].product != "0申报代理记账" &&
            _self.orderItemList[i].product != "一般纳税人代理记账"
          ) {
            _self.iscycle = true;
            break;
          }
    ...
    departnamef(value, row, index) {
      if (row.departid) {
        ...
        var servicedeparts =
          eval(row.servicedeparts) == null ? [] : eval(row.servicedeparts);

        for (var i = 0; i < servicedeparts.length; i++) {
          if (
            servicedeparts[i]["departId"] &&
    ...
    kuaiji() {
      ...
      let _isTrue = true;
      let _id = "";
      _self.$ButtonCollect("marketingManagement_index_orderlist_product_check");
      if (_self.isEdit == true) {
    ...
    fwxtj() {
      let _self = this;
      _self.$ButtonCollect("marketingManagement_index_orderlist_servicesave");
      if (_self.isCheck == true) {
      } else {
    ...
    orderEdit() {
      let _self = this;
      _self.$ButtonCollect("marketingManagement_index_orderlist_edit");
      let url = "/order/detail/" + _self.customerId;
      if (_self.customerId == "") {
        _self.$Message.warning("请选择要编辑的订单");
      } else if (
        _self.currentProcess != "Ready" &&
        _self.currentProcess != "Returned" &&
        _self.currentProcess != "ReturnedToReady"
      ) {
        ...
        function doSuccess(response) {
          ...
          for (let i = 0; i < _data.items.length; i++) {
            if (
              _data.items[i].product == "小规模代理记账" ||
              _data.items[i].product == "0申报代理记账" ||
              _data.items[i].product == "一般纳税人代理记账"
            ) {
              _self.iscycle = false;
              break;
            }
    ...
    xiugaiOpen() {
      ...
      if (_self.customerId == "") {
        ...
        function doSuccess(response) {
          ...
          _self.formValidatexiugai.nodeducttotalmoney =
            _data.nodeducttotalmoney;
    ...
    zuofei() {
      let _self = this;
      _self.$ButtonCollect("marketingManagementmarketingManagement_index_orderlist_invalid_index_edit_search");
    ...
    cksp(re) {
      let _self = this;
      _self.$ButtonCollect("marketingManagement_index_orderlist_audit");
    ...
    add_money() {
      let _self = this;
      _self.$ButtonCollect("marketingManagement_index_orderlist_money");
    ...
    setVal() {
      let _self = this;
      _self.$ButtonCollect("marketingManagement_index_orderlist_add");
    ...
    Dateok() {
      ...
      let itemGrid = "";
      let _form = "";
      _self.$ButtonCollect("marketingManagement_index_orderlist_taxperiod");
