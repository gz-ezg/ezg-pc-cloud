  ...
  <Card>
    ...
    <Row>
      <ButtonGroup style="float:left">
        <Button type="primary" icon="ios-color-wand-outline" @click="Distribution_task" v-permission="['workorder.allot']"
          >一键分配</Button
        >
...
export default {
  ...
  methods: {
      ...
      if (this.tempArray.length == 0) {
        ...
        for (let i = 0; i < this.tempArray.length; i++) {
          ...
          function success(res) {
            console.log(
              '流转成功：' + _self.tempArray[i].id + ' - ' + _self.tempArray[i].companyname + ' - ' + _self.tempArray[i].product
            );
          ...
          function fail(err) {
            console.log(
              '流转失败：' + _self.tempArray[i].id + ' - ' + _self.tempArray[i].companyname + ' - ' + _self.tempArray[i].product
            );
            if (_self.tempArray.length == i + 1) {
              // _self.getData()
              _self.$bus.emit('flowsuccess', true);
    ...
    flowChart(a) {
      ...
      _self.flowChartImg =
        '/api/dataCenter/activiti/getResourceInputStreamObj?bussinessKey=' + a.row.id + '&bussinessType=20&time=' + new Date();
    },
    // foundClues(){
