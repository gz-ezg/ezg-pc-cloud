...
<Card>
    ...
    <Row style="margin-top: 10px;">
        <Table ref="selection" highlight-row size="small" @on-row-click="selectrow" :columns="header" :data="data"></Table>
        <Page size="small" :total="pageTotal" show-total show-sizer show-elevator @on-change="pageChange" @on-page-size-change="pageSizeChange" style="margin-top: 10px"></Page>
    </Row>
    <Modal v-model="tiaozheng" title="调整服务余额" width="800">
    ...
    <Diary :row="current_row" @ok="diaryModel=false" v-if="diaryModel"></Diary>
</Card>
</template>
...
export default {
        ...
        return {
            ...
            header: [{
                                ...
                                [
                                    ...
                                    h(
                                        ...
                                        [
                                            h(
                                                "ul",
                                                this.data[params.index].CompanyName.map((item) => [
                                                    h(
                                                        "li", {
                                                            style: {
                                                                padding: "4px",
                ...
                // },
                {
                    ...
                    render: (h, params) => {
                        if (
                            params.row.workordermemo == "" ||
                            params.row.workordermemo == null
                        ) {
                            return "";
    ...
    methods: {
        ...
        showDiary() {
            if (!this.current_row) {
                return this.$Message.warning("请选择一项再进行操作");
            }
            this.diaryModel = true;
        },
        getData() {
            let _self = this;
            let url =
                "/order/cycle/service/record/list?sortField=createdate&service_type=kjdj&page=" +
                _self.page +
                "&pageSize=" +
                _self.pageSize +
                "&service_status=" +
                ...
                "&server_realname=" +
                _self.formInline.server_realname +
                "&accmanager_realname=" +
                _self.formInline.accmanager_realname +
                "&followby_realname=" +
                _self.formInline.followby_realname +
                "&begin_end_period=" +
                _self.formInline.begin_end_period +
                "&end_end_period=" +
                _self.formInline.end_end_period;
