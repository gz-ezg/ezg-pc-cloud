    ...
    <Modal v-model="model" @on-ok="$emit('ok')" @on-cancel="$emit('ok')" title="服务日志" width="80">
        <Card>
            <Collapse v-model="Collapse">
                <Panel name="1">
                    代账情况
                    <Table :loading="loading" max-height="200" slot="content" :columns="columns1" :data="info"></Table>
                </Panel>
                <Panel name="2">
                    代账工单
                    <Row slot="content">
                        <ButtonGroup style="margin-bottom:6px">
                            <Button size="small" type="primary" icon="information-circled" @click="handleAdd" v-if="isAdmin">新增税期记录</Button>
                        </ButtonGroup>
                        <Table highlight-row @on-row-click="selectWorkRow" :loading="loading" max-height="240" :columns="columns2" :data="workOrder"></Table>
                    </Row>
                </Panel>
                <Panel name="3">
                    代账月服务记录
                    <Row slot="content">
                        <ButtonGroup style="margin-bottom:6px">
                            <Button size="small" type="primary" icon="information-circled" v-if="isAdmin" @click="handleEdit">编辑</Button>
                            <Button size="small" type="primary" icon="information-circled" @click="handleDelete" v-if="isAdmin">删除</Button>
                        </ButtonGroup>
                        <Table highlight-row @on-row-click="selectRow" height="240" :loading="loading" :columns="columns3" :data="monthRecordList"></Table>
                    </Row>
                </Panel>
            </Collapse>
        </Card>
    ...
    <Modal v-model="formModel" @on-ok="handleSubmit" title="服务日志" width="400">
        <Form ref="formValidate" :model="form" :rules="ruleValidate" :label-width="80">
            ...
            <FormItem label="税期" prop="period ">
                <DatePicker @on-change="dateChange" format="yyyyMM" v-model="period" type="month" placeholder="选择税期"></DatePicker>
            </FormItem>
            ...
            <FormItem label="服务人员" prop="serverId">
                <Select v-model="form.serverId" placeholder="请选择">
                    <Option v-for="item in serverList" :key="item.serverId" :value="item.serverId">{{item.serverName}}</Option>
                </Select>
            </FormItem>
            ...
            <FormItem label="备注" prop="memo">
                <Input v-model="form.memo" type="textarea" :autosize="{minRows: 2,maxRows: 5}" placeholder="请输入备注" />
            </FormItem>
        </Form>
    </Modal>
</div>
