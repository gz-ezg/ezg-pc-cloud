<template>
	<Card>
		...
		<Row>
			<ButtonGroup>
				<Button type="primary" icon="ios-color-wand-outline" @click="stop_service" v-permission="['bookkeepingAgency.stop']">停止服务</Button>
		...
		<Row style="margin-top: 10px;">
			<Table @on-current-change="selectRow" :loading="loading" ref="selection" highlight-row size="small" :columns="header"
			 :row-class-name="rowClassName" :data="data"></Table>
			<Page size="small" :total="pageTotal" show-total show-sizer show-elevator @on-change="pageChange"
			 @on-page-size-change="pageSizeChange" :current.sync="page" placement="top" style="margin-top: 10px"></Page>
		</Row>
			...
			async getData() {
				...
				try {
					let {
						total,
						rows
					} = await accountApi.getOrderCycleMonthServiceList(config)
						...
						onOk: async () => {
							...
							try {
								let {
									status,
									data
								} = await accountApi.cycleServiceRecordUpdate(config)
								if (status) {
									_self.page = 1
									_self.getData()
								}
				...
				if (_self.img_array.length == 1) {
					...
					try {
						let {
							status,
							data
						} = await accountApi.cycleMonthServiceItemFinish(formdata)
			...
			async set_account_level() {
				...
				try {
					let {
						status,
						data
					} = await accountApi.saveAccountGradeMsg(config)
					if (status) {
			...
			async update_unit_price(index) {
				...
				try {
					let {
						status,
						data
					} = await accountApi.workOrderCycleUnitPriceUpdate(config)
					if (status) {
						this.currentIndex = -1,
							this.getData()
					}
				...
				if (!this.current_row) {
					...
					try {
						...
						let {
							status,
							data
						} = await accountApi.orderSpiderETaxDeclare(config)
						if (status) {
							this.getData()
						}
					} catch (error) {
				...
				if (!this.current_row) {
					...
					try {
						...
						let {
							status,
							data
						} = await accountApi.orderETaxSynchronizeCycleMonthServiceItemFinish(config)
						if (status) {
							this.getData()
						}
					} catch (error) {
