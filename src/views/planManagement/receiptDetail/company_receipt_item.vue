<template>
  <Modal v-model="openCompanyReceiptItem" title="企业流水" width="400">
    <div>
    ...
    get_data() {
      ...
      _self.$http.get(url, config).then(function(res) {
        ...
        for (let i = 0; i < _self.data.length; i++) {
          if (_self.currentRow.receipt_proportion && _self.data[i].amount) {
            _self.data[i].amount = (
              (_self.data[i].amount / _self.currentRow.receipt_proportion) *
              100
            ).toFixed(2);
          }
          if (_self.data[i].receipt_period) {
            _self.data[i].receipt_period = _self.data[i].receipt_period.slice(
              0,
              10
            );
          }
