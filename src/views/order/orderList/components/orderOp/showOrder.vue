          ...
          <Form ref="orderDetail" :model="orderDetail" :label-width="100" :rules="orderDetailRule">
            ...
            <Row :gutter="16">
              <div v-for="item of orderDetail.departJson">
                <Row>
                  <Col span="8">
                    <FormItem label="部门:">{{item.departName}}</FormItem>
                  </Col>
                  <Col span="15">
                    <FormItem label="人员:">{{item.realname}}</FormItem>
                  </Col>
                </Row>
              </div>
            </Row>
...
export default {
  ...
  watch: {
    orderDetail(val) {
      console.log(val);
      this.orderDetail.paydirText = this.pSimpleCodeToText(
        this.orderDetail.paydir,
        this.payDirs
      );
