    ...
    methods:{
        ...
        exportToExecl(){
            ...
            if(_that.search_companyname!=''&&_that.search_ordercode!=''){
                url = `api/order/balance/list?page=1&pageSize=${_that.pageSize}&isend=N&companyname=${_that.search_companyname}&ordercode=${_that.search_ordercode}&sumField=unpaidamount,approvalpendingamount`
            }else if(_that.search_companyname ==''&&_that.search_ordercode!=''){
                url = `api/order/balance/list?page=1&pageSize=${_that.pageSize}&isend=N&ordercode=${_that.search_ordercode}&sumField=unpaidamount,approvalpendingamount`         
            }else if(_that.search_companyname!=''&&_that.search_ordercode ==''){
                url = `api/order/balance/list?page=1&pageSize=${_that.pageSize}&isend=N&companyname=${_that.search_companyname}&sumField=unpaidamount,approvalpendingamount`         
            }else{
                url = `api/order/balance/list?page=${_that.page}&pageSize=${_that.pageSize}&isend=N&sumField=unpaidamount,approvalpendingamount`         
            }
            url = url+'exportField=[{"field": "companyName","title": "公司名称"}]'
            window.open('localhost%3a8089%2fapi%2forder%2fbalance%2flist%3fpage%3d1%26pageSize%3d9999%26isend%3dN%26sumField%3dunpaidamount%2capprovalpendingamount%26exportField%3d%5b%7b%22field%22%3a+%22companyName%22%2c%22title%22%3a+%22%e5%85%ac%e5%8f%b8%e5%90%8d%e7%a7%b0%22%7d%5d')
        }
    },
    created:function(){
        this.GetLocalData()
