      ...
      <Form :rules="ruleValidate" ref="formValidate" :model="formValidate" label-position="right" :label-width="60">
        ...
        <FormItem label="内容：">
          <div class="edit_container">
            <quill-editor class="editer" v-model="formValidate.notify_content" :options="editorOption" ref="QuillEditor">
            </quill-editor>
          </div>
        </FormItem>
        <Upload
...
<script>
import { listDepartTree, createNotify, updateNotify } from '@A/logManagement';
import { quillEditor } from 'vue-quill-editor';
import 'quill/dist/quill.core.css';
import 'quill/dist/quill.snow.css';
import 'quill/dist/quill.bubble.css';
import departTree from '@C/departTree';
...
export default {
  ...
  methods: {
    async handleConfirm(name) {
      const { notify_departs, notify_abstract, type, notify_content } = this.formValidate;
      if (!notify_content) {
    ...
    changeDepart(value) {
      ...
      this.formValidate.notify_departs = value.departId;
      this.formValidate.departName = value.departName.join(',').replace(/^,/, '');
    },
    clearDepardId() {
  ...
  async created() {
    ...
    this.formValidate.notify_content = row.notify_content;
  }
};
</script>
