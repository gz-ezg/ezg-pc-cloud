<template>
  <div>
      ...
      <Tabs v-model="openTab">
        <TabPane label="客户跟进记录" name="name8" v-if="!hideRecord">
          ...
          <Row>
            <Form ref="addDetailContent" :model="addDetailContent" :label-width="120">
              <Row :gutter="16">
                <Col span="20">
                  ...
                  >{{companyInfo.companyname}}</FormItem>
                </Col>
              </Row>
              <Row :gutter="16">
                <Col span="20">
                  ...
                  <FormItem label="通知时间：" prop="date">
                    <Col span="8">
                      <DatePicker
                        ...
                        type="date"
                        placeholder="选择日期"
                        v-model="addDetailContent.followupdate"
                        size="small"
                      ></DatePicker>
                    </Col>
                    ...
                    <Col span="8">
                      <TimePicker
                        ...
                        v-model="addDetailContent.followuptime"
                        size="small"
                      ></TimePicker>
                    </Col>
                  </FormItem>
            ...
            <Row>
              <Col :push="3">
                <Button type="primary" @click="upload" :loading="followUp_loading">新增跟进</Button>
              </Col>
            </Row>
          </Row>
          <Row>
          ...
          <Form ref="companyInfoo" :model="companyInfoo" :label-width="180" :rules="ruleValidate">
            <Row :gutter="16" style="margin-bottom: 10px">
              <Col span="8">
                <FormItem
                  ...
                  label="公司名称："
                  style="margin-bottom:5px"
                >{{companyInfoo.CompanyName}}</FormItem>
              </Col>
              <Col span="8">
              ...
              <Col span="8">
                <FormItem prop="company_type" label="企业类别：" style="margin-bottom:5px">
                  <Select type="text" size="small" v-model="companyInfoo.company_type" transfer>
                    <Option
                      v-for="(item,index) in enterprise_type"
                      :key="index"
                      :value="item.typecode"
                    >{{item.typename}}</Option>
                  </Select>
              ...
              <Col span="8">
                <FormItem prop="assets_tatol" label="资产总额：" style="margin-bottom:5px">
                  <Input
                    type="number"
                    size="small"
                    style="width: 120px;padding-right: 15px"
                    v-model="companyInfoo.assets_tatol"
                    :readonly="readonly"
                  ></Input>万元
                </FormItem>
              </Col>
              <Col span="8">
                <FormItem prop="annual_income" label="年收入或预计年收入：" style="margin-bottom:5px">
                  <Input
                    ...
                    size="small"
                    style="width: 120px;padding-right: 15px"
                    v-model="companyInfoo.annual_income"
                    :readonly="readonly"
                  ></Input>元
                </FormItem>
              </Col>
              <Col span="8">
                <FormItem prop="average_tax_burden" label="增值税、所得税平均税负：" style="margin-bottom:5px">
                  <Input
                    type="number"
                    size="small"
                    v-model="companyInfoo.average_tax_burden"
                    :readonly="readonly"
                  ></Input>
                </FormItem>
              </Col>
              ...
              <Col span="8">
                <FormItem prop="industry_tax_policy" label="行业税收政策：" style="margin-bottom:5px">
                  <Select
                    ...
                    size="small"
                    transfer
                    v-model="companyInfoo.industry_tax_policy"
                  >
                    <Option
                      v-for="(item,index) in tax_policy"
                      :key="index"
                      :value="item.typecode"
                    >{{item.typename}}</Option>
                  </Select>
              ...
              <Col span="8">
                ...
                >
                  <Select
                    ...
                    size="small"
                    transfer
                    v-model="companyInfoo.applicable_tax_rate"
                  >
                    <Option
                      v-for="(item,index) in tax_policy"
                      :key="index"
            ...
            <Row style="margin-bottom: 10px">
              <FormItem prop="business_description" style="margin-bottom:5px">
                <Input
                  v-model="companyInfoo.business_description"
                  type="textarea"
                  :autosize="{minRows: 3,maxRows: 7}"
                  :readonly="readonly"
                  style="width: 600px"
                  placeholder="对企业的主营业务、客户群体、获利能力进行描述，从票据、资金、实物（劳务）三流的来去向进行分析；画图现三流"
                ></Input>
              </FormItem>
            </Row>
            <Row style="margin-bottom: 10px">
            ...
            <Row style="margin-bottom: 10px">
              <FormItem prop="related_filing_matters" style="margin-bottom:5px">
                <Input
                  :readonly="readonly"
                  v-model="companyInfoo.related_filing_matters"
                  type="textarea"
                  :autosize="{minRows: 3,maxRows: 7}"
                  style="width: 600px"
                  placeholder="如是高新，是否享受所得税15%；是否已完成中小科技型企业备案；如是双软，是否已完成两免三减半的备案"
                ></Input>
              </FormItem>
            </Row>
            <Row style="margin-bottom: 10px">
            ...
            <Row style="margin-bottom: 10px">
              <FormItem prop="financial_handover" style="margin-bottom:5px">
                <Input
                  :readonly="readonly"
                  v-model="companyInfoo.financial_handover"
                  type="textarea"
                  :autosize="{minRows: 3,maxRows: 7}"
                  style="width: 600px"
                  placeholder="如是高新，是否享受所得税15%；是否已完成中小科技型企业备案；如是双软，是否已完成两免三减半的备案"
              ...
              <Col span="8">
                <FormItem prop="income_channel_one" label="主要收入渠道1：" style="margin-bottom:5px">
                  <Select
                    ...
                    size="small"
                    transfer
                    v-model="companyInfoo.income_channel_one"
                  >
                    <Option
                      v-for="(item,index) in zysrqd"
                      :key="index"
                      :value="item.typecode"
                    >{{item.typename}}</Option>
                  </Select>
              ...
              <Col span="8">
                <FormItem prop="income_channel_two" label="主要收入渠道2：" style="margin-bottom:5px">
                  <Select
                    ...
                    size="small"
                    transfer
                    v-model="companyInfoo.income_channel_two"
                  >
                    <Option
                      v-for="(item,index) in zysrqd"
                      :key="index"
            ...
            <Row :gutter="12" style="margin-bottom: 10px">
              <Col span="8">
                <FormItem prop="customer_scale_type" label="主要客户规模及类型：" style="margin-bottom:5px">
                  <Input
                    size="small"
                    v-model="companyInfoo.customer_scale_type"
                    :readonly="readonly"
                  ></Input>
                </FormItem>
              </Col>
              <Col span="8">
                <FormItem prop="three_description_one" label="三流描述：" style="margin-bottom:5px">
                  <Input
                    size="small"
                    v-model="companyInfoo.three_description_one"
                    :readonly="readonly"
                  ></Input>
                </FormItem>
              </Col>
            ...
            <Row :gutter="12" style="margin-bottom: 10px">
              <Col span="8">
                <FormItem prop="supplier_scale_type" label="主要供应商规模及类型：" style="margin-bottom:5px">
                  <Input
                    size="small"
                    v-model="companyInfoo.supplier_scale_type"
                    :readonly="readonly"
                  ></Input>
                </FormItem>
              </Col>
              <Col span="8">
                <FormItem prop="three_description_two" label="三流描述：" style="margin-bottom:5px">
                  <Input
                    size="small"
                    v-model="companyInfoo.three_description_two"
                    :readonly="readonly"
                  ></Input>
                </FormItem>
              </Col>
            ...
            <Row style="margin-bottom: 10px">
              <FormItem prop="company_point_service" style="margin-bottom:5px">
                <Input
                  :readonly="readonly"
                  v-model="companyInfoo.company_point_service"
                  type="textarea"
                  :autosize="{minRows: 3,maxRows: 7}"
                  style="width: 600px"
                  placeholder="详细描述企业服务特征"
                ></Input>
              </FormItem>
            </Row>
            ...
            <Scroll height="500">
              ...
              <Form ref="mainInfo" :model="mainInfo" :label-width="120">
                ...
                <Row :gutter="16">
                  ...
                  <Col span="10">
                    <FormItem
                      ...
                      label="营业状态："
                      style="margin-bottom:5px"
                    >{{mainInfo.busnissstatus}}</FormItem>
                  </Col>
                  <Col span="10">
                    <FormItem
                      prop="issueDate"
                ...
                <Row :gutter="24">
                  ...
                  <Col span="20">
                    <FormItem
                      ...
                      label="登记机关："
                      style="margin-bottom:5px"
                    >{{mainInfo.registerOffice}}</FormItem>
                  </Col>
                </Row>
                <Row :gutter="24">
                  <Col span="1" style="visibility:hidden">1</Col>
                ...
                <Row :gutter="24">
                  ...
                  <Col span="20">
                    <FormItem
                      prop="permissionShopType"
                      label="许可经营范围："
                      style="margin-bottom:5px"
                    >{{mainInfo.permissionShopType}}</FormItem>
                  </Col>
                </Row>
                <Row :gutter="24">
                  <Col span="1" style="visibility:hidden">1</Col>
                ...
                <Row :gutter="16">
                  <Col span="8">
                    <FormItem
                      ...
                      label="公司名称："
                      style="margin-bottom:5px"
                    >{{realationPersonTitle.CompanyName}}</FormItem>
                  </Col>
                  <Col span="8">
                    <FormItem
                      prop="companyname"
                      label="客户名称："
                      style="margin-bottom:5px"
                    >{{realationPersonTitle.NAME}}</FormItem>
                  </Col>
                  <Col span="8">
                    <FormItem
                      prop="companyname"
                      label="电话："
                      style="margin-bottom:5px"
                    >{{realationPersonTitle.TEL}}</FormItem>
                  </Col>
                </Row>
                <Row :gutter="16">
                  <Col span="8">
                    <FormItem
                      ...
                      label="固话："
                      style="margin-bottom:5px"
                    >{{realationPersonTitle.fixedPhone}}</FormItem>
                  </Col>
                  <Col span="8">
                    <FormItem
                      prop="companyname"
                      label="QQ："
                      style="margin-bottom:5px"
                    >{{realationPersonTitle.qq}}</FormItem>
                  </Col>
                  <Col span="8">
                    <FormItem
                      prop="companyname"
                      label="微信："
                      style="margin-bottom:5px"
                    >{{realationPersonTitle.weixin}}</FormItem>
                  </Col>
                </Row>
                <Row :gutter="16">
                  <Col span="8">
                    <FormItem
                      ...
                      label="邮箱："
                      style="margin-bottom:5px"
                    >{{realationPersonTitle.email}}</FormItem>
                  </Col>
                </Row>
              </Form>
            </Row>
            ...
            <Row :gutter="16" style="margin-bottom:16px">
              ...
              <Col span="11">
                <FormItem prop="nationalnum" label="电子税务局账号：" style="margin-bottom:5px">
                  <Input type="text" v-model="taxManagement.nationalnum" :disabled="isEditTax"></Input>
                </FormItem>
              </Col>
              <Col span="11">
                <FormItem prop="nationalpsw" label="电子税务局密码：" style="margin-bottom:5px">
                  <Input type="text" v-model="taxManagement.nationalpsw" :disabled="isEditTax"></Input>
                </FormItem>
              </Col>
            </Row>
            <Row :gutter="16" style="margin-bottom:16px">
              ...
              <Col span="11">
                <FormItem prop="accounttype" label="账号类型：" style="margin-bottom:5px">
                  <Select
                    transfer
                    v-model="taxManagement.accounttype"
                    placeholder
                    :disabled="isEditTax"
                  >
                    <Option
                      v-for="(item, index) in etax_account_type"
                      :key="index"
                      :value="item.typecode"
                    >{{item.typename}}</Option>
                  </Select>
            ...
            <Row :gutter="16" style="margin-bottom:16px">
              ...
              <Col span="11">
                <FormItem prop="nationalnum" label="税号：" style="margin-bottom:5px">
                  <Input type="text" v-model="taxManagement.tax_number" :disabled="isEditTax"></Input>
                </FormItem>
              </Col>
              <Col span="11">
                <FormItem prop="nationalpsw" label="个税密码：" style="margin-bottom:5px">
                  <Input type="text" v-model="taxManagement.tax_password" :disabled="isEditTax"></Input>
                </FormItem>
              </Col>
            </Row>
            <Row :gutter="16" style="margin-bottom:16px">
              ...
              <Col span="11">
                <FormItem prop="nationalnum" label="税控盘密码：" style="margin-bottom:5px">
                  <Input type="text" v-model="taxManagement.taxdisk_password" :disabled="isEditTax"></Input>
                </FormItem>
              </Col>
              <Col span="11">
                <FormItem prop="nationalpsw" label="CA密码：" style="margin-bottom:5px">
                  <Input type="text" v-model="taxManagement.ca_password" :disabled="isEditTax"></Input>
                </FormItem>
              </Col>
            </Row>
            <Row :gutter="16" style="margin-bottom:16px">
              ...
              <Col span="11">
                <FormItem prop="nationalnum" label="红盾网密码：" style="margin-bottom:5px">
                  <Input
                    type="text"
                    v-model="taxManagement.redshield_password"
                    :disabled="isEditTax"
                  ></Input>
                </FormItem>
              </Col>
    ...
    <Modal v-model="contentdetail" title="跟进详情" width="700" @on-cancel="cancelContent">
        ...
        <Row :gutter="16">
          ...
          <Col span="20">
            <FormItem
              ...
              label="跟进类型："
              style="margin-bottom:5px"
            >{{detailContent.typename}}</FormItem>
          </Col>
        </Row>
        <Row :gutter="16">
          <Col span="1" style="visibility:hidden">1</Col>
        ...
        <Row :gutter="16">
          ...
          <Col span="20">
            <FormItem
              ...
              label="跟进人："
              style="margin-bottom:5px"
            >{{detailContent.realname}}</FormItem>
          </Col>
        </Row>
        <Row :gutter="16">
          <Col span="1" style="visibility:hidden">1</Col>
          <Col span="20">
            <FormItem
              ...
              label="跟进时间："
              style="margin-bottom:5px"
            >{{detailContent.createdate}}</FormItem>
          </Col>
        </Row>
        <Row :gutter="16" v-if="imgsList.length">
          <Col span="1" style="visibility:hidden">1</Col>
          <FormItem prop="createdate" label="沟通证据：" style="margin-bottom:5px">
            <a
              span="8"
              style="max-width:200px;display:inline-block;margin-right:10px"
              v-for="(url_img, imgIndex) in imgsList"
              :key="imgIndex"
              target="_blank"
              :href="'api/assets/' + url_img"
            >
              <img
                :src="'api/assets/' + url_img"
                alt="图片"
                width="95%"
    ...
    <Modal v-model="addcontentdetail" title="新增跟进" width="700" @on-cancel="canceladdContent">
      <Form ref="addDetailContent" :model="addDetailContent" :label-width="120">
        <Row :gutter="16">
          <Col span="1" style="visibility:hidden">1</Col>
          <Col span="20">
...
import {
  servicePeriod,
  findFollowContentImgageByBusinessId,
} from "@A/order.js";
...
export default {
  ...
  data() {
    return {
      ...
      ruleTaxManagement: {
        nationalnum: [{ required: true, message: "必填项！", trigger: "blur" }],
        nationalpsw: [{ required: true, message: "必填项！", trigger: "blur" }],
        accounttype: [
          { required: true, message: "必填项！", trigger: "change" },
        ],
        validflag: [{ required: true, message: "必填项！", trigger: "change" }],
      },
      companyInfoo: {
        industry_type: "",
        company_type: "",
      ...
      ruleValidate: {
        ...
        ifMatch: [{ required: true, message: "请选择", trigger: "change" }],
        servicestartdate: [
      ...
      followUpHeader: [
        ...
        },
        {
          ...
          render: (h, params) => {
            // console.log(params)
            if (
              params.row.service_content == "" ||
              params.row.service_content == null
            ) {
              return "";
  ...
  methods: {
    checkMonth(data) {
      ...
      if (this.servicestartdate) {
        ...
        let between =
          data.getFullYear() * 12 + data.getMonth() - a * 12 - b * 1;
    ...
    t(e) {
      ...
      this.notify_ids = this.notify_ids.substring(
        0,
        this.notify_ids.length - 1
      );
        ...
        if (valid) {
          ...
          let config = {
            ...
            customerServiceStandard:
              _self.companyInfoo.customer_service_standard,
    ...
    getData() {
      ...
      let config = {
        params: {
          id: _self.companyId,
          detailTypes:
            "basic,main,workOrder,taxManagement,customerRecord,dynamic,companyInfo",
        },
      };
      _self.detail = true;
      function success(res) {
        _self.companyInfo =
          res.data.data.basic.length != 0 ? res.data.data.basic[0] : {};
        _self.mainInfo =
          res.data.data.main.length != 0 ? res.data.data.main[0] : {};
        _self.workOrder = res.data.data.workOrder;
        _self.taxManagement =
          res.data.data.taxManagement.length != 0
            ? res.data.data.taxManagement[0]
            : {};
        _self.followUpData = res.data.data.customerRecord;
        _self.realationPerson = res.data.data.dynamic;
        _self.realationPersonTitle =
          res.data.data.customer.length != 0 ? res.data.data.customer[0] : {};
        _self.companyInfoo =
          res.data.data.companyInfo.length != 0
            ? res.data.data.companyInfo[0]
            : {};
    ...
    async openContent(e) {
      ...
      const resp = await findFollowContentImgageByBusinessId({
        bussinessId: e.row.bussinessId,
      });
      if (resp[0].imgurls) {
    ...
    add_workorder_followup() {
      ...
      let config = {
        ...
        notifyDate:
          DateFormat(_self.addDetailContent.followupdate) +
          " " +
          _self.addDetailContent.followuptime,
    ...
    GetFollowUpType() {
      ...
      let params =
        "follow_up_type,financialLevel,customerrating,etax_account_type";
      ...
      function success(res) {
        ...
        for (let i = 0; i < res.data.data.follow_up_type.length; i++) {
          var temp = {};
          if (
            res.data.data.follow_up_type[i].typecode == 21 ||
            res.data.data.follow_up_type[i].typecode == 22
          ) {
          } else {
    ...
    getRole() {
      ...
      if (
        _self.$indexOfArray(temp, ["kuaiji", "shangshi", "qihua", "shenji"])
      ) {
    ...
    get_data_center() {
      let params =
        "industry_category,enterprise_type,tax_policy,sfgxjsr,kffwbz,zysrqd";
