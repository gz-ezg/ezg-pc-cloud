    ...
    async get_data() {
      ...
      try {
        ...
        this.data = rows.map((item) => {
          ...
          item.contract_flag = this.order_contract_flag_map.get(
            item.contract_flag
          );
    ...
    async get_data_center() {
      ...
      try {
        let {
          ...
          cluesources,
          order_contract_flag,
        } = await refundApi.getDictionary(params);
        ...
        this.order_contract_flag_map = this.$array2map(
          this.order_contract_flag
        );
      } catch (error) {
        console.log(error);
      }
