<template>
  <div>
    <Modal v-model="isOpenAdd" title="录入" width="800" @on-cancel="close">
        ...
        <Row :gutter="16">
          ...
          <Col span="2">
            <p style="padding-top:8px">
              共 <span style="color:red">{{ defaultData.length }}</span> 种
            </p>
          </Col>
        </Row>
        <Row :gutter="16">
      ...
      <div slot="footer">
        <Button type="primary" @click="submit" :disabled="openSubmit">新增</Button>
    ...
    <Modal v-model="selectProduct" title="选择产品" width="300">
      <Row :gutter="16">
        <Col span="20">
    ...
    <Modal v-model="followbysTag" title="选择服务人员" width="500">
      <Row :gutter="16">
        <Col span="12">
      ...
      <Table style="margin-top: 10px" highlight-row :columns="columns3" @on-row-dblclick="rowSelect3" :data="data3"></Table>
      <Page
        size="small"
        :total="pageTotala"
        show-total
...
export default {
  ...
  created() {
    ...
    _self.$bus.on('OPEN_OFFLINE_ADD', e => {
      ...
      _self.task_message.enddate = new Date();
