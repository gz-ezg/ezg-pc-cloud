<template>
    <div style="min-width:1300px" @click="close_right_menu">
        <Button v-if="right_click_show" :style="{top: rightTop + 'px', left: rightLeft + 'px'}" style="position:fixed;z-index:9000" type="primary" @click="add_task">新增普通外勤任务</Button>
        <Card title="普通外勤任务">
    ...
    export default {
        ...
        methods:{
            ...
            eventSelected(event, jsEvent, view){
                //  点击展示事件详情
                this.$bus.emit("OPEN_NORMAL_DETAIL_TASK",event)
            },
            ...
            add_task(){
                let _self = this
                this.$bus.emit("SCHEDULE_CREATE_NORMAL_TASK", _self.rightClickDate)
            },
            next() {
        ...
        created() {
            ...
            _self.$bus.on("UPDATE_NORMAL_TASK_LIST_DEMO", (e)=>{
                _self.get_data()
                _self.get_onedate_data(DateFormat(new Date()))
            })
            _self.$bus.on("UPDATE_NORMAL_TASK_LIST",(e)=>{
                _self.load = true
                _self.get_data()
                _self.get_onedate_data(DateFormat(new Date()))
            })
